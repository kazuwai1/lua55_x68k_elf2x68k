...
#else /* __human68k__ */
/* using x68k's history.x */
static void lua_initreadline (lua_State *L) {
	/* do nothing */
}

#define lua_readline(buff,prompt)	((void)buff, x68_readline(prompt))

static struct dos_inpptr cmdlinebuf;
static int last_readline_status = 0;

char * x68_readline( const char * prompt ) {
	char *chkptr, c;

	if ( last_readline_status == 1 )
		return NULL;

	if ( lua_stdin_is_tty() ) {	/* tty */
		_dos_print(prompt);

		cmdlinebuf.max = 255;
		cmdlinebuf.buffer[0] = 0;
		_dos_gets( &cmdlinebuf );
		_dos_print("\n");
